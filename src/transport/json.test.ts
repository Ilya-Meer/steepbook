...
describe('Export to JSON', () => {
  ...
  it('calls downloadFile with correctly formatted parameters', () => {
    ...
    const mockSessions: Session[] = [
      {
        ...
        steeps: ['Wash', 'Fruity', 'Floral', ''], // Include empty steep to test filtering
        customFields: [
          { name: 'Water Temperature', value: '100' },
          { name: 'Rating', value: '9/10' },
          { name: 'Empty Field', value: '' } // Test filtering empty custom fields
        ]
      },
      {
        ...
        steeps: ['some value here', 'another value here'], // Different number of steeps
        customFields: [
          { name: 'Water Temperature', value: '100°C' },
          { name: 'Tea Pet', value: 'Lord GuanYu' }, // Different custom field
          { name: 'Rating', value: '8/10' }
        ]
      }
    ]
    ...
    expect(firstSession.steeps).toEqual(['Wash', 'Fruity', 'Floral'])
    ...
    expect(firstSession.customFields).not.toContainEqual({ name: 'Empty Field', value: '' })
    ...
    expect(secondSession.steeps).toEqual(['some value here', 'another value here'])
    ...
    expect(secondSession.customFields).toEqual([
      { name: 'Water Temperature', value: '100°C' },
      { name: 'Tea Pet', value: 'Lord GuanYu' },
      { name: 'Rating', value: '8/10' }
    ])
    ...
    const mockSessions: Session[] = [
      {
        ...
        additionalNotes: 'Very refreshing',
        steeps: ['', '', ''], // Only empty steeps
        customFields: [
          { name: 'Empty Field 1', value: '' },
          { name: 'Empty Field 2', value: '' }
  ...
  it('preserves mixed empty and non-empty values correctly', () => {
    const mockSessions: Session[] = [
      {
        ...
        wetLeaf: 'Bright green, expanded',
        additionalNotes: 'Very refreshing',
        steeps: ['Wash', '', 'Floral', ''], // Mixed empty and non-empty
        customFields: [
          { name: 'Water Temperature', value: '80°C' },
          { name: 'Empty Field', value: '' },
          { name: 'Rating', value: '9/10' },
          { name: 'Another Empty', value: '' }
        ]
      }
    ]
    ...
    expect(session.steeps).toEqual(['Wash', 'Floral'])
